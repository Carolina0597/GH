...
function EncuestasGH({
  ...
  const [creatingTipo, setCreatingTipo] = useState<TipoEncuesta | null>(null)
  ...
  if (viewMode === 'editor') {
    return <EncuestaEditor encuesta={editingEncuesta} tipoNueva={creatingTipo} onBack={handleBack} onSave={() => handleBack()} />
  }
    ...
    <div className="space-y-6">
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="max-w-2xl bg-surface border-border">
          <DialogHeader>
            <DialogTitle>Que tipo de encuesta quieres crear?</DialogTitle>
            <DialogDescription>Selecciona el tipo de evaluacion que necesitas configurar.</DialogDescription>
          </DialogHeader>
          <div className="grid sm:grid-cols-2 gap-3 py-4 max-h-[60vh] overflow-y-auto">
            {Object.entries(tipoEncuestaLabels).map(([tipo, label]) => (
              <div key={tipo} onClick={() => { setCreatingTipo(tipo as TipoEncuesta); setEditingEncuesta(null); setShowCreateDialog(false); setViewMode('editor') }}
                className="p-4 rounded-lg bg-surface-2 border border-border hover:border-primary/50 cursor-pointer transition-all">
                <div className="flex items-center gap-2 mb-2"><FileText className="w-5 h-5 text-primary" /><span className="font-medium">{label}</span></div>
                <p className="text-xs text-muted-foreground">{getTipoDesc(tipo)}</p>
              </div>
            ))}
          </div>
          <div className="flex items-center gap-2 p-3 rounded-lg bg-primary/10 border border-primary/30">
            <Sparkles className="w-5 h-5 text-primary" />
            <div className="flex-1"><p className="text-sm font-medium">Generar con IA</p><p className="text-xs text-muted-foreground">La IA generara una propuesta de encuesta.</p></div>
            <Button variant="outline" size="sm" className="border-primary/50 text-primary">Usar IA</Button>
          </div>
        </DialogContent>
      </Dialog>
